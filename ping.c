...
{ 
    ...
    total_msec = ((tfe.tv_sec-tfs.tv_sec)*1000.0+ timeElapsed)/100.0;
                     
    printf("\n===%s ping statistics===\n", ping_ip); 
    printf("\n%d packets sent, %d packets received, %f percent packet loss. Total time: %Lf ms. Total ping time: %Lf ms.\n\n",  msg_count, msg_received_count, ((msg_count - msg_received_count)/msg_count) * 100.0, total_msec, total_rtt_msec);
} 
